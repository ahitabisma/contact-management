...
import { detail, update } from "../../lib/api/contact";
import { errorHandler } from "../../lib/api/errorHandler";
import { alertConfirm, alertError, alertSuccess } from "../../lib/alert";
import { useState } from "react";
import { useAuthStore } from "../../stores/auth";
import { deleteAddress, list } from "../../lib/api/address";
...
export default function Detail() {
  ...
  async function fetchContact() {
    try {
      const response = await detail(token.token, id);
  ...
  async function fetchAddresses() {
    try {
      const response = await list(token.token, id);
  ...
  async function handleDeleteAddress(addressId) {
    try {
      const isDelete = await alertConfirm(
      ...
      await deleteAddress(token.token, id, addressId);
